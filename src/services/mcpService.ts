  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        renewMonths: { type: 'number', description: '（可选）续期周期月数，不设置时自动按循环间隔续期：每几天→天数、每几个月→月数、每几年→年数、每周→7天、每年→12个月' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        renewMonths: { type: 'number', description: '（可选）续期周期月数，不设置时自动按循环间隔续期' },
      },
      required: ['id'],
    },
  },
