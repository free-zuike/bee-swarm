...
export class MigrationService {
  ...
  private async migrationRemoveDuplicateCacheColumns(): Promise<void> {
    try {
      ...
      // 2. 定义我们要保留的所有列（排除冗余的缩写字段）
      const columnsToKeep = columns.filter(col => 
        !['cache_ttl_b', 'cache_ttl_c', 'cache_ttl_t', 'cache_ttl_g', 'cache_ttl_s'].includes(col)
      );

      // 3. 动态构建 CREATE TABLE 语句
      // 首先获取每列的类型和其他信息
      const columnDefs: string[] = [];
      for (const col of result.results) {
        if (!['cache_ttl_b', 'cache_ttl_c', 'cache_ttl_t', 'cache_ttl_g', 'cache_ttl_s'].includes(col.name)) {
      ...
      if (columnDefs.length === 0) {
        ...
        createTableSql = `CREATE TABLE IF NOT EXISTS users_new (${columnDefs.join(', ')})`;
      }

      await this.env.DB.prepare(createTableSql).run();

      // 4. 复制数据（只保留我们需要的列）
      const insertSql = `
        INSERT INTO users_new (${columnsToKeep.join(', ')})
