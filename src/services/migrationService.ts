...
export class MigrationService {
  ...
  private async migrationRemoveDuplicateCacheColumns(): Promise<void> {
    try {
      // 暂时禁用自动删除冗余字段
      // 我们通过管理页面提供手动清理功能，避免自动操作导致问题
      console.log('[Migration] Skipping automatic cache column cleanup for safety');
      console.log('[Migration] You can manually clean up redundant columns through the admin page if needed');
      return;
    } catch (error) {
      console.error('[Migration] Failed in cache column cleanup check:', error);
    }
  }
